...
const prettyJson = JSON.stringify(person, null, 4);
...
const filteredJson = JSON.stringify(
    //변환할 객체
    person,
    //replacer 함수 : 변환시에 필터링하는 함수, 원하는 프라퍼티만 반환하기 위해서
    //문자열로 반환하면서  undifined값은 뺌
    (key, value) => {
...
// 요청 초기화
// open(HTTP요청메소드, 요청 URL)
// HTTP요청 메소드 : GET, POST, PUT, PATCH, DELETE
//                               GET : 요청헤더만 전송, 조회
//                               POST : 요청헤더와 요청바디를 전송, 등록
//                               PUT : 요청헤더와 요청바디를전송, 데이터 전체 수정 
//                               PATCH : 요청헤더와 요청바디를전송, 데이터 부분 수정
//                               DELETE : 요청헤더만 전송, 데이터 삭제
xhr.open('GET', 'http://jsonplaceholder.typicode.com/todos/1');

// 요청 전송
xhr.send();
// onreadtstatechange: readtstate 프라퍼티의 값이 변경될때마다 실행될 함수를 저장하는 프라퍼티
...
xhr.onreadystatechange = () => {
    ...
    // 서버 응답이 정상완료되면
    if (xhr.status == 200) {
        // 서버에서 보낸 응답body를 JSON객체로 변환해서 출력
        console.log(typeof xhr.response, xhr.response);
        //응답 body JSON문자열을 객체로 변환
