...
keyInput.addEventListener('keydown', event =>{
    keyOutput.textContent = `입력된 키: ${event.key}`;//아스키코드값
});
...
mouseArea.addEventListener('mouseleave',()=>{
    mouseOutput.textContent ='마우스가 영역서 벗어났습니다!'
});
...
alertBtn1.addEventListener('click', handleClick);
//클릭하면 alerBtn의 이벤트리스너를 제거
alertBtn2.addEventListener('click', ()=> {
    alertBtn1.removeEventListener('click',handleClick);
});